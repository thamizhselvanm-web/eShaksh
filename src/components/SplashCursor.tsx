...
export const SplashCursor: React.FC<SplashCursorProps> = ({
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas: HTMLCanvasElement) {
      ...
      let formatRGBA = getSupportedFormat(gl, isWebGL2 ? (gl as WebGL2RenderingContext).RGBA16F : gl.RGBA, gl.RGBA, halfFloatTexType) || { internalFormat: gl.RGBA, format: gl.RGBA, type: gl.UNSIGNED_BYTE };
      let formatRG = getSupportedFormat(gl, isWebGL2 ? (gl as WebGL2RenderingContext).RG16F : gl.RGBA, isWebGL2 ? (gl as WebGL2RenderingContext).RG : gl.RGBA, halfFloatTexType) || { internalFormat: gl.RGBA, format: gl.RGBA, type: gl.UNSIGNED_BYTE };
      let formatR = getSupportedFormat(gl, isWebGL2 ? (gl as WebGL2RenderingContext).R16F : gl.RGBA, isWebGL2 ? (gl as WebGL2RenderingContext).RED : gl.RGBA, halfFloatTexType) || { internalFormat: gl.RGBA, format: gl.RGBA, type: gl.UNSIGNED_BYTE };
    ...
    const displayShaderSource = `
      ...
      void main () {
          ...
          c *= 0.5;
          float a = max(c.r, max(c.g, c.b)) * 0.28;
    ...
    function splat(x: number, y: number, dx: number, dy: number, color: any) {
      ...
      gl.uniform1f(splatProgram.uniforms.aspectRatio, canvas!.width / (canvas!.height || 1));
    ...
    function getResolution(resolution: number) {
      let aspectRatio = gl!.drawingBufferWidth / (gl!.drawingBufferHeight || 1);
